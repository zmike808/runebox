...
class UnusedArgRemover : Transformer {
    ...
    override fun run(group: ClassGroup) {
        ...
        group.classes.forEach { cls ->
            cls.methods.forEach methodLoop@{ method ->
                val identifier = findOverride(method.owner.name, method.name, method.desc, rootMethods, classNames)
                    ?: return@methodLoop
        ...
        group.classes.forEach { cls ->
            cls.methods.forEach { method ->
                method.instructions.forEach insnLoop@{ insn ->
                    ...
                    val identifier = findOverride(insn.owner, insn.name, insn.desc, opaqueMethods.keys, classNames)
                        ?: return@insnLoop
    ...
    private val MethodNode.lastArgIndex: Int
        get() {
    ...
    private fun AbstractInsnNode.isIntConstant() = when (opcode) {
        ...
        Opcodes.SIPUSH, Opcodes.BIPUSH, Opcodes.ICONST_M1, Opcodes.ICONST_0, Opcodes.ICONST_1, Opcodes.ICONST_2, Opcodes.ICONST_3, Opcodes.ICONST_4, Opcodes.ICONST_5 -> true
        else -> false
    }
    ...
    private fun findSupers(cls: ClassNode, classNames: Map<String, ClassNode>): Collection<ClassNode> {
        return cls.interfaces.plus(cls.superName).mapNotNull { classNames[it] }
            .flatMap { findSupers(it, classNames).plus(it) }
    }
    ...
    private fun findOverride(
        ...
        desc: String,
        methods: Set<String>,
        classNames: Map<String, ClassNode>
    ): String? {
