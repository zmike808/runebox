...
import io.runebox.mixin.annotation.CLocalVariable;
...
public class CInjectAnnotationHandler extends RemovingTargetAnnotationHandler<CInject> {
    ...
    @Override
    public void transform(CInject annotation, MixinManager mixinManager, Map<String, IInjectionTarget> injectionTargets, ClassNode transformedClass, ClassNode transformer, MethodNode transformerMethod, MethodNode target) {
        boolean hasArgs;
        boolean hasCallback;
                        ...
                        if (paramAnnotations != null) {
                            for (AnnotationNode paramAnnotation : paramAnnotations) {
                                if (paramAnnotation.desc.equals(typeDescriptor(CLocalVariable.class))) {
                                    CLocalVariable localVariable = AnnotationParser.parse(CLocalVariable.class, mixinManager, AnnotationParser.listToMap(paramAnnotation.values));
