    ...
    public void translate(AnnotationNode annotation) {
        ...
        if (values.containsKey("value") && values.containsKey("target")) {
            ...
            if (value.equalsIgnoreCase("CONSTANT")) {
                ...
                if (!(opcode instanceof Integer)) throw new IllegalArgumentException("RuneBox Mixins requires an opcode to be specified");
                values.put("value", "OPCODE");
                values.put("target", opcode);
            } else if (value.equalsIgnoreCase("FIELD")) {
                if (values.containsKey("opcode")) {
                    throw new IllegalArgumentException("RuneBox Mixins does not support the FIELD target with opcode. Please refer to the At#opcode javadoc for information");
                }
            }
        }
